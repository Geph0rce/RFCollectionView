...
//
#ifndef _EXTSYNTHESIZE_H_
#define _EXTSYNTHESIZE_H_

#import <RFFoundation/EXTRuntimeExtensions.h>
...
/**
 * \@synthesizeAssociationCType synthesizes a scalar property for a class using NSValue and associated
 * objects. This is primarily useful for adding properties to a class within
 * a category.
 *
 * PROPERTY must have been declared with \@property in the interface of the
 * specified class (or a category upon it), and must be of Ctype.
 *
 */
#define synthesizeAssociationCType(CLASS, PROPERTY, TYPE) \
dynamic PROPERTY; \
\
static void *ext_uniqueKey_ ## CLASS ## _ ## PROPERTY = &ext_uniqueKey_ ## CLASS ## _ ## PROPERTY; \
\
...
static void ext_ ## CLASS ## _ ## PROPERTY ## _synthesize (void) { \
    ...
    id getter = ^(id self){ \
         TYPE cValue = { 0 }; \
         NSValue *value = objc_getAssociatedObject(self, ext_uniqueKey_ ## CLASS ## _ ## PROPERTY); \
         if (value != nil) { \
    ...
    id setter = ^(id self, TYPE value){ \
         NSValue *object = [NSValue value:&value withObjCType:@encode(TYPE)]; \
         objc_setAssociatedObject(self, ext_uniqueKey_ ## CLASS ## _ ## PROPERTY, object, policy); \
    }; \
    \
    char *getterTypes = (char *)[[@(@encode(TYPE)) stringByAppendingString:@"@:"] UTF8String];\
    if (!class_addMethod(cls, attributes->getter, imp_implementationWithBlock(getter), getterTypes)) { \
        NSCAssert(NO, @"Could not add getter %s for property %@.%s", sel_getName(attributes->getter), cls, # PROPERTY); \
    } \
    \
    if (!class_addMethod(cls, attributes->setter, imp_implementationWithBlock(setter), "v@:@")) { \
